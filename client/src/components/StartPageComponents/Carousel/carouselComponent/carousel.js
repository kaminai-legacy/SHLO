...
const functionInterval=()=>{
    timeout = setInterval(() => {
        ...
        setLeft(left-378*countPictureForSlide)}else{
    ...
    const slides=FIRST_CAROUSEL.map((element)=>{
        return<div key={element} className={style.carouselItem}  onMouseOver={()=>clearInterval(timeout)}  onMouseOut={()=>functionInterval()}
