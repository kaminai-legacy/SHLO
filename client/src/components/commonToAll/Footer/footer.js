...
import {listFirst,hoizontLine,listSecond} from '../../../constants/footerContent';
...
const listsToRenderSecondLine=(lists)=>{
  return lists.map((item,id)=>{
      ...
      <div className={style.listElements}>
      {item.elements.map((element,ident)=>{
        return<Link key={ident} to={element.link}><div  className={style.listElement}>
          {element.text}
        </div></Link>
      })}
      </div>
...
const  footer=() =>{
  ...
  {listSecondToRender}
           ...
           <Link to="https://www.shopperapproved.com/reviews/squadhelp.com/"> <div className={style.raiting}>Squadhelp.com has a Shopper Approved rating of 4.9/5 based on 2472 ratings and reviews</div></Link>
       </div>
    </div>
